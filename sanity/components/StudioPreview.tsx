...
export default function StudioPreview({ document, options }: { document: { displayed: any }, options?: { type?: string, mode?: 'card' | 'normal' } }) {
    ...
    const resolveColor = (colorStr: string) => {
        ...
        // Common Tailwind/CSS Names (Synced with lib/tags.ts)
        const colors: Record<string, string> = {
            emerald: '#34d399', // emerald-400
            green: '#4ade80',   // green-400
            blue: '#60a5fa',    // blue-400
            red: '#ef4444',     // red-500 (kept as warning color)
        ...
        return colors[colorStr.toLowerCase()] || colorStr // Fallback to raw string (might be a valid CSS color)
    }

    // CARD MODE: Render only the card preview
    if (options?.mode === 'card') {
        return (
            <Card padding={4} height="fill" tone="transparent" style={{
                background: '#050505',
                color: '#fff',
                backgroundImage: `
            ...
            `,
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'auto' }}>
                        <span style={{
                            color: resolveColor(doc.color) || '#fff',
                            borderColor: resolveColor(doc.color) || 'rgba(255,255,255,0.2)',
                            borderWidth: '1px',
                            borderStyle: 'solid',
                            fontSize: '11px',
                            fontWeight: 'bold',
    ...
    // NORMAL MODE: Render generic layout (removed inline card)

    // Generic field mapping
    const title = doc.title || doc.name || 'Untitled'
    // Prioritize specific fields, fallback to generic ones
    const summary = doc.summary || doc.description || doc.bio || doc.subtitle
    // Portable Text content (posts only usually)
    const content = doc.content

    // Details for varied types
    const extraDetails = []
    if (doc.slug?.current) extraDetails.push({ label: doc.slug.current })
    if (doc.tag) extraDetails.push({ label: doc.tag, title: 'Tag' })
    // If we have a specific color, add a badge for it AND use it for the others potentially? 
    // For now, let's just make the color badge itself colored if it's a hex
    if (doc.color) extraDetails.push({ label: doc.color, color: doc.color, title: 'Color' })
                        ...
                        {extraDetails.length > 0 && (
                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                {extraDetails.map((detail, i) => {
                                    const resolvedColor = resolveColor(detail.color)
                                    const bgColor = resolvedColor || 'rgba(255,255,255,0.1)'
                                    const textColor = resolvedColor ? '#fff' : '#a1a1aa'

                                    // Special handle for light backgrounds if needed, but for now assuming dark/vibrant colors
                                    ...
                                    return (
                                        <div key={i} style={{
                                            ...
                                            <span style={{
                                                ...
                                                fontWeight: 600,
                                                textShadow: resolvedColor ? '0 1px 2px rgba(0,0,0,0.3)' : 'none'
                                            }}>
                                                {detail.label}
                                            </span>
                                        </div>
